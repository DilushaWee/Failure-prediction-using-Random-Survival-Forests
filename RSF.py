...
class RandomSurvivalForest():
    def __init__(self, n_trees=100, max_features=2, max_depth=5, min_samples_split=2, split="auto"):
    ...
    #     print((compute_trees),'compute_trees')
    #     # print(sum(compute_trees) / self.n_trees,'ff')
    #     print((np.percentile(compute_trees, (100 - 95) / 2.)),'percentile')
    ...
    def predict_proba(self, x):
        ...
        # compute_trees = [x.apply(lambda u: self.predict_row(self.trees[i], u), axis=1) for i in range(self.n_trees)]
        # print(x.iloc[1,:],'iii')
        # print(self.n_trees,'trees')

        # for i in range(0,2):
        #     print(i,'te')
            # compute_trees= [x.iloc[0,:].apply(lambda u: self.predict_row(self.trees[i], u), axis=1)
        list_perc_upper=[]
        list_perc_lower=[]
        list_failure_prediction=[]
        for training_data in range(0,len(x)):
         sum_trs = []
         for i in range(self.n_trees):
                compute_trees=self.predict_row(self.trees[i], x.iloc[training_data,:])
                sum_trs.append(compute_trees)


         # print(sum_trs, 'sum of compute_trees')
        # print(sum(compute_trees) / self.n_trees,'ff')
         perc_up=(np.percentile(sum_trs, (100 - 95) / 2.))
         perc_lower=(np.percentile(sum_trs, 100 - (100 - 95) / 2.))
         # print(perc_up,perc_lower)
         list_perc_upper.append(perc_up)
         list_perc_lower.append(perc_lower)
         list_failure_prediction.append(sum(sum_trs) / self.n_trees)
        return list_perc_upper,list_perc_lower,list_failure_prediction
